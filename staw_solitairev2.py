...
def shuffle_ships(list_data):
    """shuffe_ships(list) returns a randomize list items."""
...
def movement_phase(list_data, game_round):
    """movement_phease(list, int) returns a list of ships to move in succession."""
    shuffle_ships(list_data)
    print(f"\nMovement Phase Round {game_round}")
    return generate_ship_list(list_data, "move")


def attack_phase(list_data, game_round):
    """attack_phase(list, int) returns a list of ship to attack in succession."""
    ship_down = True
    shuffle_ships(list_data)
    print(f"\nAttack Phase Round {game_round}")
    ...
    if user_response == 'y':
        while ship_down:
            print("Enter the dstroyed ship's index number or x to exit:")
...
def end_phase(game_round):
    ...
    print("2.) Remove all remaining Scan, Evade, Battlestations & red Cloak tokens.")
...
def game_setup():
    """game_setup() accepts user input, setup the data structure for the game."""
